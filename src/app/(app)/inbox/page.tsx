...
export default async function InboxPage() {
  ...
  const [sentRows, waiting, answerRows, replyRows] = await Promise.all([
    prisma.application.findMany({
      ...
      take: 200,
      select: {
        id: true, status: true, portal: true, submittedVia: true, submitConfirmation: true, answersUsedJson: true,
        pendingQuestionsJson: true, createdAt: true, completedAt: true, lastReplyAt: true, lastReplyKind: true, replyCount: true, userStatus: true,
  ...
  const sent: InboxSent[] = sentRows.map((a) => ({
    ...
    pending: parse<{ label: string }>(a.pendingQuestionsJson).map((q) => q.label),
