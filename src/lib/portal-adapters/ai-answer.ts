...
export async function answerRequiredFields(
...
): Promise<{ answered: number; remainingRequired: number; details: string[] }> {
  ...
  try {
    ...
    return { answered: 0, remainingRequired: 0, details: ["collect_failed"] };
  }
  if (pending.length === 0) {
    return { answered: 0, remainingRequired: 0, details: ["no_required_empty"] };
  }

  // 2. Per i react-select, apri e leggi le opzioni (best-effort) così l'AI
  //    può scegliere tra valori reali.
  ...
  // 3. Chiedi a Claude le risposte (solo dai dati reali).
  let answers: AiAnswer[] = [];
  try {
    answers = await askClaude(ctx, pending);
  } catch (err) {
  ...
  for (const f of pending) {
    ...
    if (ok) {
      answered++;
      details.push(`${f.kind}:"${f.label.slice(0, 40)}"=${String(val).slice(0, 30)}`);
    }
  }

  // 5. Ricalcola quanti required-vuoti restano (per decidere UNCONFIRMED).
  let remainingRequired = 0;
  try {
    remainingRequired = (await collectRequiredEmptyFields(page)).length;
  } catch {
    remainingRequired = pending.length - answered;
  }

  return { answered, remainingRequired, details };
}
