 ...
 */
export type DiscoveredTenants = Record<"personio" | "teamtailor" | "recruitee" | "bamboohr", string[]>;
...
export async function discoverTenantsFromPool(): Promise<DiscoveredTenants> {
  const out: DiscoveredTenants = { personio: [], teamtailor: [], recruitee: [], bamboohr: [] };
  try {
    ...
    const sets: Record<string, Set<string>> = { personio: new Set(), teamtailor: new Set(), recruitee: new Set(), bamboohr: new Set() };
    for (const r of rows) {
      let host = "";
